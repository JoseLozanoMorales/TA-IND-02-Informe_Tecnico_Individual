...
El teorema CAP formaliza que, cuando se produce una partición de red, un sistema replicado no puede garantizar simultáneamente disponibilidad para todas las solicitudes y una vista consistente de los datos \cite{gilbert2002brewer}. La formulación no significa que un sistema elija permanentemente dos letras de tres; describe la decisión que debe tomar durante una partición.

PACELC amplía el análisis: si existe una partición, se elige entre disponibilidad y consistencia; de lo contrario, en operación normal, se elige entre latencia y consistencia \cite{abadi2012pacelc}. Dynamo representa una decisión orientada a disponibilidad y convergencia eventual \cite{decandia2007dynamo}, mientras Spanner emplea coordinación y sincronización temporal para ofrecer consistencia externa global \cite{corbett2013spanner}. En una instancia única, CAP no se manifiesta como un conflicto entre réplicas: ante la pérdida de acceso al único nodo, el servicio queda indisponible. PACELC tampoco presenta todavía un intercambio de latencia por coordinación entre copias, aunque sí existen costos de bloqueo y contención entre transacciones.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \draw[dashed,-{Latex[length=2mm]}] (pg.east) -- node[above,sloped]{separación lógica futura} (future.west);
  \end{tikzpicture}
...
La primera alternativa conserva una sola fuente autorizada para escrituras y añade uno o más seguidores mediante streaming replication. El checkout, inventario, pagos y pedidos se ejecutan en el primario. Las consultas tolerantes a obsolescencia, como catálogo o reportes, pueden dirigirse a un seguidor. Estudios experimentales muestran que los niveles de consistencia elegidos en bases NoSQL y distribuidas afectan directamente latencia y rendimiento \cite{ferreira2025benchmarking}; por ello, el enrutamiento debe decidirse por operación, no globalmente.
...
La Tabla~\ref{tab:comparacion} muestra que la alternativa A ofrece el mejor equilibrio inmediato. Las arquitecturas recientes también demuestran que es posible combinar replicación, consistencia fuerte y alto rendimiento mediante diseños especializados \cite{krechowicz2025layered}, pero su complejidad no se justifica todavía en TiendaTech. La consistencia eventual estudiada en entornos de sincronización distribuida \cite{zhao2025acc} resulta más apropiada para datos tolerantes a retraso que para la confirmación de inventario y cobro.
...
Para la preparación de este informe se utilizaron Claude y OpenAI Codex como herramientas de apoyo. Claude fue empleado previamente para organizar y mejorar la claridad de un borrador inicial. OpenAI Codex se utilizó para inspeccionar la estructura del repositorio de TiendaTech, localizar evidencia de configuración de base de datos y transacciones, contrastar dicha evidencia con la documentación previa, proponer la estructura en LaTeX y revisar la coherencia técnica del texto. Las herramientas asistieron en el resumen, marco teórico, análisis, tabla comparativa y redacción. La selección de la postura, la validación de las afirmaciones sobre el PFC y la responsabilidad por el contenido final corresponden al autor, quien debe revisar el documento antes de entregarlo.

\bibliographystyle{IEEEtran}
\bibliography{fuentes_primarias}
